  ...
  if ( !any(is.na(randomize)) )  {
    ...
    if ( length(block)==0 ) reorder <- order(runif(dim(data)[1])) else {
      ...
      for (i in bi) reorder[bdat==i] <- 
        reorder[bdat==i][order(runif(sum(bdat==i)))] #dim(reorder[bdat==i,])[1]))]
  ...
  if ( adapt ) { # Fill in data
    p.list <- p.list.dmc(p.vector,model=model,n1order=FALSE,cvs=cv.out,
         cells=paste(apply(data[,names(attr(model,"factors")),drop=FALSE],1,
              paste,collapse="."),attributes(model)$responses[1],sep="."),
        #cells=attr(cvs, 'row.facs'),
        do.trans=TRUE)
    # Factors 
    attr(p.list,"facs") <- data[,fnams,drop=FALSE]
    attr(p.list,"cvs") <- cv.out
    adapt.out <- random.dmc(p.list,model,save.adapt=TRUE)
    
    ## SM
    data[,colnames(adapt.out)] <- adapt.out  # overwrite rowfacs if possible
