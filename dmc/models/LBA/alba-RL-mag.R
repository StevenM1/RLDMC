...
transform2.dmc <- function(pars, cvs, choiceIdx) {
  ...
  mean_v <- cbind(pars[1,,"V0"] + pars[1,,"wV"]*(pars[2,,"SR"]-pars[1,,"SR"]) + pars[1,,'wS'] * (pars[1,,'SR']+pars[2,,'SR']),
                  pars[2,,"V0"] + pars[2,,"wV"]*(pars[1,,"SR"]-pars[2,,"SR"]) + pars[2,,'wS'] * (pars[1,,'SR']+pars[2,,'SR']))
...
random.dmc <- function(p.list,model,save.adapt=TRUE)
{
  ...
  out <- rLBA(n=n,
              ...
              b=list(pars[1,,'A']+b[,1], pars[2,,'A']+b[,2]),
