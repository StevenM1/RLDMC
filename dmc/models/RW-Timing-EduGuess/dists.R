...
rWaldRace_Timing_NS <- function(n, t0T, t0E, v_E, v_T, s_E, s_T, B_T, B_E, A, Qvalues, st0 = 0, silent=TRUE, simPerTrial=FALSE){
  ...
  p.guess <- apply(Qvalues, 2, function(x) exp(x[1])/(exp(x[1]) + exp(x[2])))
  if(any(timer.replace)) {
    ...
    out$R[timer.replace] <- sapply(p.guess[timer.replace], function(x) sample(c(1, 2), size = 1, prob = c(x, 1-x)))
    browser()
  }
  out
}
...
n1WaldTiming_NS <- function(dt,B,A,t0, B_T, v_T, s_T, A_T, t0T, Qvalues, gf=0, ...)
  # Generates defective PDF for responses on node=1, dt (decison time) is a vector of times
{
  ...
  Qvalues <- check_n1_arguments(Qvalues, nn=nn, n_v = n_v)
  #  B[B<0] <- 0 # Protection for negatives
  #  A[A<0] <- 0
  ...
  if (!is.matrix(Qvalues)) Qvalues <- do.call(rbind, Qvalues)
  p.guess <- apply(Qvalues, 2, function(x) exp(x[1])/(exp(x[1]) + exp(x[2])))
  ...
  if(t0T[[1]] < t0E[[1]]) {
    ...
    if(any(u)) {  # evidence process hasn't started so must be a timer response with a guess
      ...
      dt[1, u] <- dWald(dt[1,u], v=v_T[1,u], B=B_T[1,u], A=A_T[1,u], s=s_T[1,u]) * p.guess[u]
    }
    if(any(!u)) { # could be a response from the timer or evidence process
      ...
      pdf_T <- dWald(dt[1,!u], v=v_T[1,!u], B=B_T[1,!u], A=A_T[1,!u], s=s_T[1,!u]) * p.guess[!u]
    ...
    if(any(!u)) { # could be a response from the timer or evidence process
      ...
      #Timing accumulator = density Timing * pguess * cumulative distribution function of E accumulators
      pdf_T <- dWald(dt[1,!u]-d[!u], v=v_T[1,!u], B=B_T[1,!u], A=A_T[1,!u], s=s_T[1,!u]) * p.guess[!u]
...
dWaldRace <- function(rt, response, A, B, t0, B_T, v_T, s_T, A_T, t0T, ..., 
                      st0 = 0, Qvalues, args.dist = list(), silent = FALSE)
{
  dots <- list(...)
  ...
  dots$v <- check_i_arguments(dots$v, nn = nn,
                              n_v = n_v, dots = TRUE)
  Qvalues <- check_i_arguments(Qvalues, nn = nn, n_v = n_v)
  ...
  for (i in unique(response)) {
    ...
    out[sel] <- do.call(n1WaldTiming_NS, args = c(dt = list(rt[sel]),
                                                  ...
                                                  Qvalues = list(lapply(Qvalues, '[', i=sel)[c(i, seq_len(n_v)[-i])]),
                                                  gf=0,
