...
random.dmc <- function(p.list,model,save.adapt=FALSE)
{
  ...
  choiceIdx <- apply(cvs[,1:ncol(cvs)], 1, function(x) which(!is.na(x)))
  ...
  #  learningRates[,ncol(cvs)] <- pars[1,1,'aB']  # last column = feedback on accuracy
  ...
  #  estimatedAccuracy <- updated$adaptedValues[,ncol(cvs)]
  b <- cbind(pars[1,,"B0"],# * (1+pars[1,,"wB"]*(-.25+pars[1,,"SR"]*(1-pars[1,,"SR"]))),
             pars[2,,"B0"])# * (1+pars[2,,"wB"]*(-.25+pars[2,,"SR"]*(1-pars[2,,"SR"]))))

  v0 <- cbind(pars[1,,"V0"] * (1+pars[1,,"wB"]*(-.25+pars[1,,"SR"]*(1-pars[1,,"SR"]))),
              pars[2,,"V0"] * (1+pars[2,,"wB"]*(-.25+pars[2,,"SR"]*(1-pars[2,,"SR"]))))

  mean_v <- cbind(v0[,1] + pars[1,,"wV"]*(pars[2,,"SR"]-pars[1,,"SR"]),
                  v0[,2] + pars[2,,"wV"]*(pars[1,,"SR"]-pars[2,,"SR"]))
  
  #  pars[,,"B0"] = pars[,,"B0"]*(1 + pars[,,"wB"]*(updated$adaptedValues[,ncol(cvs)]-.5))
  ...
  if(save.adapt) {
    ...
    for(i in 1:dim(pars)[1]) { # accumulator N
      ...
      for(parName in c('SR')) { # parameter N}
        ii <- which(dimnames(pars)[[3]]==parName) #parName <- dimnames(pars)[3][[1]][ii]
      ...
      adapt[,paste0('v0.', accumulatorName)] <- v0[,i]
    }
    adapt[,'PEs'] <- apply(updated$predictionErrors[,1:(ncol(cvs)-1)], 1, sum, na.rm=TRUE)
    #    adapt[,'estimatedAccuracy'] <- estimatedAccuracy
  }
...
likelihood.dmc <- function(p.vector,data,min.like=1e-10, use.c=TRUE)   
{
  ...
  if(use.c) {
    ...
    startValues <- attr(data, "startingValues")#, .5)
