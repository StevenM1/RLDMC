...
random.dmc <- function(p.list,model,save.adapt=FALSE)
{
  ...
  startValues <- cbind(rep(pars[1,1,'SR'], ncol(cvs)-1), .5)
  ...
  v0 <- cbind(pars[1,,"V0"] * (1+pars[1,,"wB"]*(estimatedAccuracy-.5)),
              pars[2,,"V0"] * (1+pars[2,,"wB"]*(estimatedAccuracy-.5)))
...
likelihood.dmc <- function(p.vector,data,min.like=1e-10, use.c=TRUE)   
{
  ...
  if(use.c) {
    ...
    startValues <- c(attr(data, "startingValues"), .5)
