...
{
  par.df$aR[(par.df$aR == Inf) | (par.df$aR == -Inf)] = par.df$aV[(par.df$aR == Inf) | (par.df$aR == -Inf)]
...
transform2.dmc <- function(pars, cvs, choiceIdx) {
  ...
  riskStartValues <- c(rep(pars[1,,'RR'], each=2, times=ncol(cvs)-1), 1)  # no risk for accuracy learning?
  
  # learning rates matrix
  learningRates <- cbind(matrix(rep(pars[1,,'aV'], each=ncol(cvs)-1), ncol=ncol(cvs)-1, byrow=TRUE),
                         pars[1,,'aB'])
  riskLearningRates <- cbind(matrix(rep(pars[1,,'aR'], each=ncol(cvs)-1), ncol=ncol(cvs)-1, byrow=TRUE),
                             0)
