...
transform2.dmc <- function(pars, cvs, choiceIdx) {
  ...
  mean_v <- cbind(pars[1,,"V0"] + pars[1,,"wV"]*exp(pars[2,,"SR"]/pars[1,,"SR"]),
                  pars[2,,"V0"] + pars[2,,"wV"]*exp(pars[1,,"SR"]/pars[2,,"SR"]))
...
likelihood.dmc <- function(p.vector,data,min.like=1e-10, use.c=TRUE)   
{
  ...
  if(any(is.na(mean_v)) | any(is.na(b))) {
    return(rep(min.like, nrow(data)))
  }
