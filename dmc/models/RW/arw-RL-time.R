...
random.dmc <- function(p.list,model,save.adapt=TRUE)
{
  ...
  learningRatesPos <- matrix(pars[1,,'aV'], nrow=nrow(cvs), ncol=ncol(cvs))
#  learningRatesNeg <- matrix(pars[1,,'aVN'], nrow=nrow(cvs), ncol=ncol(cvs))
  ...
  nTrials <- 1:nrow(cvs)/nrow(cvs)
  
#  b0*(1-1:nT*(wB/nT))
  v0 <- cbind(pars[1,,"V0"] * (1+nTrials*pars[1,,"wV0"]),
              pars[2,,"V0"] * (1+nTrials*pars[2,,"wV0"]))
  # add back data to 'pars' array
  # v0 <- cbind(pars[1,,"V0"] - pars[1,,"wV0"]*(1:nrow(cvs)*pars[1,,"V0"]),
  #             pars[2,,"V0"] - pars[2,,"wV0"]*(1:nrow(cvs)*pars[2,,"V0"]))
  
  pars[,,'SR'] <- matrix(updated$adaptedValues[choiceIdx], ncol=2, byrow=FALSE)
  mean_v <- cbind(v0[,1] + pars[1,,"wV1"]*(pars[2,,"SR"]-pars[1,,"SR"]),
                  v0[,2] + pars[2,,"wV1"]*(pars[1,,"SR"]-pars[2,,"SR"]))
  
  b <- cbind(pars[1,,"B0"] * (1+nTrials*pars[1,,"wB"]),
             pars[2,,"B0"] * (1+nTrials*pars[2,,"wB"]))
  
  s <- cbind(pars[1,,'s'] * (1+nTrials*pars[1,,'ws']),
             pars[2,,'s'] * (1+nTrials*pars[2,,'ws']))
  ...
  if(save.adapt) {
    ...
    adapt$PEs <- apply(updated$predictionErrors, 1, sum, na.rm=TRUE)
    adapt$s <- s[,1]
  }
...
transform.dmc <- function(par.df,do.trans=TRUE) 
{
 ...
 if(!'ws' %in% names(par.df)) par.df$ws <- matrix(0, ncol=ncol(par.df$B0), nrow=nrow(par.df$B0))
  if (do.trans)
    list(A=t(par.df$A),s=t(par.df$s),t0=t(par.df$t0),st0=t(par.df$st0),
         B0=t(par.df$B0),
         SR=t(par.df$SR),
         wB=t(par.df$wB),
         aV=pnorm(t(par.df$aV)),
         wV0=t(par.df$wV0),
         wV1=t(par.df$wV1),
         ws=t(par.df$ws),
#         aVN=pnorm(t(par.df$aVN)),
         V0=t(par.df$V0)) else
           list(A=par.df$A,s=par.df$s,t0=par.df$t0,st0=par.df$st0,
                ...
                wV0=par.df$wV0,
                ws=par.df$ws,
                aV=t(pnorm(t(par.df$aV))),
#                aVN=t(pnorm(t(par.df$aVN))),
                V0=par.df$V0,wV1=par.df$wV1)       
}
...
likelihood.dmc <- function(p.vector,data,min.like=1e-10, use.c=TRUE)   
{
  ...
  if(use.c) {
    ...
    startValues <- rep(pars[1,1,'SR'], ncol(cvs))  #attr(data, "startingValues")
    #  startValues <- attr(data, "startingValues")
    learningRatesPos <- matrix(pars[1,,'aV'], nrow=nrow(cvs), ncol=ncol(cvs))
#    learningRatesNeg <- matrix(pars[1,,'aVN'], nrow=nrow(cvs), ncol=ncol(cvs))
  ...
  # Update parameters: linear increase/decrease in V0 (as function of TRIAL)
  v0 <- cbind(pars[,1,"V0"] * (1+nTrials*pars[,1,"wV0"]), #*(1:nrow(cvs)*pars[,1,"V0"]),
              pars[,2,"V0"] * (1+nTrials*pars[,2,"wV0"])) #pars[,2,"wV0"]*(1:nrow(cvs)*pars[,2,"V0"]))
  
  # mean_v changes as function of learning/ev
  mean_v <- cbind(v0[,1] + pars[,1,"wV1"]*(pars[,2,"SR"]-pars[,1,"SR"]),
                  v0[,2] + pars[,2,"wV1"]*(pars[,1,"SR"]-pars[,2,"SR"]))
  
  # Update parameters: linear increase/decrease in B0 (as function of TRIAL)
  b <- cbind(pars[,1,"B0"] * (1+nTrials*pars[,1,"wB"]), # pars[,1,"wB"]*(1:nrow(cvs)*pars[,1,"B0"]),
             pars[,2,"B0"] * (1+nTrials*pars[,2,"wB"])) #pars[,2,"wB"]*(1:nrow(cvs)*pars[,2,"B0"]))
