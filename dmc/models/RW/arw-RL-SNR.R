...
transform2.dmc <- function(pars, cvs, choiceIdx) {
  ...
  ## for now, assume SNR is "known"
  noise <- sqrt(pars[,,'SR']*(1-pars[,,'SR']))
  snr <- pars[,,'SR']/noise
  
  # Determine parameter mean_v
  mean_v <- cbind((pars[1,,"V0"] + pars[1,,"wV"]*(pars[2,,"SR"]-pars[1,,"SR"])) * (snr[1,]^2 + snr[2,]^2),
                  (pars[2,,"V0"] + pars[2,,"wV"]*(pars[1,,"SR"]-pars[2,,"SR"])) * (snr[1,]^2 + snr[2,]^2))
...
likelihood.dmc <- function(p.vector,data,min.like=1e-10, use.c=TRUE)   
{
  ...
  if(any(mean_v < 0.05)) return (rep(min.like, nrow(data)))
