...
transform2.dmc <- function(pars, cvs, choiceIdx) {
  ...
  mean_v <- cbind(pars[1,,"V0"] + pars[1,,"wV"]*pars[2,,"SR"]*pars[2,,'RR'],
                  pars[2,,"V0"] + pars[2,,"wV"]*pars[1,,"SR"]*pars[1,,'RR'])
  ...
  return(list(pars=pars, mean_v=mean_v, b=b, s=s, updated=updated))
}
...
random.dmc <- function(p.list,model,save.adapt=TRUE)
{
  ...
  if(save.adapt) {
    ...
    for(i in 1:dim(pars)[1]) { # accumulator N
      ...
      adapt[,paste0('s.', accumulatorName)] <- s[,i]
    }
...
likelihood.dmc <- function(p.vector,data,min.like=1e-10, use.c=TRUE)   
{
  ...
  if(any(is.na(s))) return(rep(min.like, nrow(data)))
  if(any(s < 0.01)) return(rep(min.like, nrow(data)))
  # likelihood
