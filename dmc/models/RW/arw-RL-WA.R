...
transform2.dmc <- function(pars, cvs, choiceIdx) {
  ...
  pars[,,'SR'] <- matrix(updated$adaptedValues[choiceIdx], ncol=3, byrow=FALSE)
  ...
  b <- cbind(pars[1,,"B0"], pars[2,,"B0"], pars[3,,"B0"])
  
  return(list(pars=pars, inputs=inputs, b=b, updated=updated))
}
...
random.dmc <- function(p.list,model,save.adapt=TRUE)
{
  ...
  if(!is.null(attr(cvs, "VVchoiceIdx"))) {
    choiceIdx <- attr(cvs, 'VVchoiceIdx')
  } else {
...
#      adapt[,paste0('mean_v.', accumulatorName)] <- mean_v[,i]
    ...
    Input = names (inputs) = 1:nOptions
    advantages <- t(apply(inputs, 1, ToM))
    sums <- t(apply(inputs, 1, ToP))
    v = pars[1,,'V0'] + pars[1,,'wS']*sums + pars[1,,'wV']*advantages
    tmp <- 1:3; names(tmp) <- 1:3 
    colnames(v) <- names(ToM(tmp))  # useful
    adapt <- cbind(adapt, v)
  }
  ...
  dimnames(pars)[[3]] <- c("A", "s", "t0" , "st0", "B", "SR", "aV", "V0", "wV",  "wS")
  out <- lapply(1:nrow(inputs), function(x) WArnd(x=pars[1,x,], N=3, Trials=1, Inputs=inputs[x,]))
...
likelihood.dmc <- function(p.vector,data,min.like=1e-10, use.c=TRUE)   
{
  ...
  pmax(WAlikevec(RT=data$RT,
                 ...
                 v0=pars[1,1,'V0'], 
                 ws=pars[1,1,'wS'],
                 wd=pars[1,1,'wV']), min.like, na.rm=TRUE)
  # pmax(sapply(1:nrow(data), function(x) WApdf(data[x,'RT'], inputs[x,], as.numeric(data[x,'R']), 
  #                                             t0=pars[1,1,'t0'], 
  #                                             A=pars[1,1,'A'], 
  #                                             B=pars[1,1,'B0'], 
  #                                             v0=pars[1,1,'V0'], 
  #                                             ws=pars[1,1,'wS'],
  #                                             wd=pars[1,1,'wV'], sl = 0)), min.like, na.rm=TRUE)
...
#  if(!'choiceIsHighP' %in% colnames(dat)) stop('Column `choiceIsHighP` is missing')
  ...
  outcomes <- matrix(NA, nrow=nrow(df), ncol=length(unique(df$stimulus_set))*3)
  for(row in 1:nrow(df)) {
    cond = df$stimulus_set[row]
    outcomes[row,(cond)*3+df[row, 'choice']] <- df$reward[row]
  }
  ...
  for(tr in 1:nrow(df)) {
    ...
    VVchoiceIdx[tr, ((stimulus_set)*3+1):((stimulus_set)*3+3)] <- TRUE
  }
  VVchoiceIdx <- which(t(VVchoiceIdx), arr.ind = TRUE)[,2:1]  # Gives for every trial each column that is chosen
  return(list(df=df, VVchoiceIdx=VVchoiceIdx, outcomes=outcomes))
}
