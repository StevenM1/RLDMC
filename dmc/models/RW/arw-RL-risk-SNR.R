...
transform2.dmc <- function(pars, cvs, choiceIdx) {
  ...
  # snr <- pars[,,'SR'] / sqrt(pars[,,'RR'])
  # mean_snr <- 
  mean_risk <- pars[1,,'RR'] + pars[2,,'RR']  # sum variances
  
  # Determine parameter mean_v
  mean_v <- cbind((pars[1,,"V0"] + pars[1,,"wV"]*(pars[2,,"SR"]-pars[1,,"SR"]))/mean_risk,
                  (pars[2,,"V0"] + pars[2,,"wV"]*(pars[1,,"SR"]-pars[2,,"SR"]))/mean_risk)
