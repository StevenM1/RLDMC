  ...
  if (do.trans) {
    return(
      list(A=t(par.df$A),
           ...
           V0Mod=t(par.df$V0Mod),
           B0Mod=t(par.df$B0Mod),
           t0Mod=t(par.df$t0Mod),
           aVMod=t(par.df$aVMod),
           wS=t(par.df$wS)))
  } else {
    return(
...
transform2.dmc <- function(pars, cvs, choiceIdx) {
  ...
  learningRates <- matrix(rep((1+pars[1,,'aVMod'])*pars[1,,'aV'], each=ncol(cvs)), ncol=ncol(cvs), byrow=TRUE)
  ...
  mean_v <- cbind((1+pars[1,,'driftMod']) * ( (1+pars[1,,'V0Mod'])*pars[1,,"V0"] + pars[1,,"wV"]*(pars[2,,"SR"]-pars[1,,"SR"]) + pars[1,,'wS'] * (pars[2,,"SR"]+pars[1,,"SR"])),
                  (1+pars[2,,'driftMod']) * ( (1+pars[2,,'V0Mod'])*pars[2,,"V0"] + pars[2,,"wV"]*(pars[1,,"SR"]-pars[2,,"SR"]) + pars[2,,'wS'] * (pars[2,,"SR"]+pars[1,,"SR"])))
  
  # Determine parameter b
  b <- cbind((1+pars[1,,'B0Mod'])*pars[1,,"B0"], 
             (1+pars[2,,'B0Mod'])*pars[2,,"B0"])
...
random.dmc <- function(p.list,model,save.adapt=TRUE)
{
  ...
  out <- rWaldRaceSM(n=n,
                     ...
                     B=list(b[,1], b[,2]),
                     t0=list((1+pars[1,,'t0Mod'])*pars[1,,'t0'], 
                             (1+pars[2,,'t0Mod'])*pars[2,,'t0']),
                     st0=0,
