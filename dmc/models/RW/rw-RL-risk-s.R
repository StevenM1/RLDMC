...
transform.dmc <- function(par.df,do.trans=TRUE) 
{
  ...
  par.df$RR <- t(pnorm(t(par.df$RR)))
  par.df$RR = par.df$RR / 2       ## max is 0.5 for binomial distribution!
...
transform2.dmc <- function(pars, cvs, choiceIdx) {
  ...
  mean_v <- cbind((pars[1,,"V0"] + pars[1,,"wV"]*(pars[2,,"SR"])),
                  (pars[2,,"V0"] + pars[2,,"wV"]*(pars[1,,"SR"])))
  
  # Determine parameter b
  b <- cbind(pars[1,,"B0"],  # + pars[1,,"wB"]*pars[1,,"RR"]*pars[1,,"B0"],
             pars[2,,"B0"])  # + pars[2,,"wB"]*pars[2,,"RR"]*pars[2,,"B0"])
...
likelihood.dmc <- function(p.vector,data,min.like=1e-10, use.c=TRUE)   
{
  ...
  if(any(mean_v < 0.01)) return(rep(min.like, nrow(data)))
  if(any(is.na(pars[,,'RR']))) return(rep(min.like, nrow(data)))
  if(any(pars[,,'RR'] < 0.01)) return(rep(min.like, nrow(data)))
