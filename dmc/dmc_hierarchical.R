...
h.simulate.dmc <- function(model,ps=NA,ns=1,n=2,randomize=NA,block=NA,
                           ...
                           subject.cv=NULL,by.cell=TRUE,save.adapt=FALSE)
...

{
  ...
  dati <- simulate.dmc(ps[1,],modeli,n[1,],randomize=randomize,block=block,cvs=cvs[[1]],
      SSD=SSD[datr],staircase = staircase,by.cell=by.cell,save.adapt=save.adapt)
  if (save.adapt) {
...
h.post.predict.dmc <- function(samples,n.post=100,probs=c(1:99)/100,bw="nrd0",
  save.simulation=FALSE,factors=NA,save.subject.posts=FALSE,adapt=FALSE,
  ...
  # apply post.predict to each subject
{
  ...
  if ( cores>1 & length(samples)>1 & os=="windows") { 
    ...
    out <- mclapply(samples,post.predict.dmc,n.post=n.post,probs=probs,bw=bw,
                    factors=factors,save.simulation=save.simulation,gglist=TRUE,
                    adapt=adapt,
                    save.simulation.as.attribute=TRUE,ignore.R2=ignore.R2,censor=censor, mc.cores=cores)
  } else {
