...
source ("dmc/dmc.R")
source('utils.R')
load_model ("RW", "arw-RL-risk-mag.R")

# some names
modelName = 'arw-RL-risk-mag-SAT-B'
...
for(dataName in c('SAT')) {
  ...
  p.vector  <- c(t0=.2, aV=-1.6, wS=1, aR=-1.6,
  ...
  #### Sample  -----------------------------------------------------------------
  doSample(data, p.prior, pp.prior, nmcBurn=300, nCores=15, restart=FALSE, useRUN=TRUE, fileName=file.path(samplesDir, fn))
}


### Plots & checks ----------
samples <- loadSamples(fn, samplesDir)
samples <- h.samples.dmc(nmc=0,add=TRUE,samples=samples,remove=1:1000)
