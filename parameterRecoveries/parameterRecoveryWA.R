...
#       thisTrial <- WArnd(x=c('v0'=v0, 'wD'=wV, 'wS'=wS, 't0'=t0, 'B'=B, 'A'=0), 3, Trials=1, Inputs = values)
...
# # dataName <- 'parameterRecovery-expMultiAlternative'
...
# setProbability <- list(c(0.15, .15, 0.7),
#                        c(0.2, 0.2, 0.6),
#                        c(0.25, 0.25, 0.5),
#                        c(0.15, .15, 0.7),
#                        c(0.2, 0.2, 0.6),
...
#   t0 = medianPars$t0[s]
#   data <- simulate.WA(nTrialsPerSet = 52, nSets=9, 
...
# save(allData, trueParameters, file = './parameterRecoveries/data/parameterRecovery-expMultiAlternative.RData')
# 
...
model <- model.dmc(p.map=list(A="1",t0="1",st0="1",s="1",
                   ...
                   type="norm")
load('./parameterRecoveries/data/parameterRecovery-expMultiAlternative.RData')
...
for(sub in unique(allData$s)) {
  d <- prepareForFittingMA(allData[allData$s==sub,])
  cvs[[sub]] <- d$outcomes
...
fn = paste0('model-', modelName, '_data-', dataName)
#### Sample  -----------------------------------------------------------------
doSample(data, p.prior, pp.prior, nmcBurn=250, nCores=30, restart=FALSE, fileName=file.path(samplesDir, fn))
...
source ("dmc/dmc.R")
source('utils.R')
load('./parameterRecoveries/samples/model-arw-RL-WA_data-parameterRecovery-expMultiAlternative.RData')
load('./parameterRecoveries/data/parameterRecovery-expMultiAlternative.RData')
...
# pdf(file='figures/MA-parrec-apriori.pdf', width=6, height=4)
