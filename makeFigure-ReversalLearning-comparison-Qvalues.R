...
allBPICs <- cbind(BPICDDM[,2],  BPICARDf[,2])
...
pp3 <- tmp[['pp3']]   # make sure this is based on the RL-fARD
...
# #  plot.new()
# #  if(i == 0) mtext('RL-DDM', side=3, cex=.66*1.2, font=2, line=1)
# #  if(i == 1) {mtext('RL-fARD', side=3, cex=.66*1.2, font=2, line=1)}
#   i <- i+1
# #  par(mar=c(0,2,1,.5)+.1)
#   plotDataPPBins(data=qRTs$meanAccOverTime[[1]], pp=qRTs$meanAccOverTime[[2]],
...
#   } else {
#     mtext('RL-fARD', side=3, cex=.66*1.2, font=2, line=1)
