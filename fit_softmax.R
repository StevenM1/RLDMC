...
for(dataName in c('calibration', 'annie-chris', 'calibration-subset')) {
  ...
  #### Sample  -----------------------------------------------------------------
  doSample(data, p.prior, pp.prior, nmcBurn=150, nCores=15, restart=TRUE, fileName=fn)
}
...
# # load('calibration_softmax.Rdata')#; samples <- burn1
...
nBins <- 10
if(!'ease' %in% colnames(dat)) dat$ease <- as.factor(abs(dat$high_stim_prob-dat$low_stim_prob))
...
plot.pp.dmc(ppNoSim, style='cdf', x.min.max = c(0, 2), no.layout = TRUE, do.main=FALSE); title('Overall def. CDFs')
plotDataPPBins(data=meanRTsOverTime[[1]], pp=meanRTsOverTime[[2]], ylim=c(0.6, 0.8)); title('Mean RT over time')
...
for(ease in unique(meanRTsByEase[[1]]$ease)) {
  ...
  plot.pp.dmc(ppNoSimSubset, style='cdf', no.layout = TRUE, do.main=FALSE, x.min.max=c(0,2)); title(paste0('Ease ', ease))

  idxD = meanRTsByEase[[1]]$ease == ease
  idxM = meanRTsByEase[[2]]$ease == ease
  plotDataPPBins(data=meanRTsByEase[[1]][idxD,], pp=meanRTsByEase[[2]][idxM,], draw.legend=FALSE, ylim=c(0.5, 0.8)); title('Mean RT over time')
...
for(i in 1:nBins) {
  ...
  ppNoSimSubset <- h.pp.summary(pp2Subset, samples=samples, dat=dataSubset, ignore.subjects=TRUE)
  plot.pp.dmc(ppNoSimSubset, style='cdf', no.layout = TRUE, do.main=FALSE, aname='', x.min.max=c(0,3)); title(paste0('Bin ', i))
}
...
for(i in 1:length(ppNoSim)) {
  plot.pp.dmc(ppNoSim[[i]], style='cdf', x.min.max = c(0, 3), no.layout = TRUE, do.main=FALSE); title(paste0('Participant ', i)) #Overall def. CDFs')
  plotDataPPBins(data=meanRTsBySub[[1]][[i]], pp=meanRTsBySub[[2]][[i]], ylim=c(.3, 2)); title('Mean RT over time')
...
# meanV1OverTimeM <- aggregate(mean_v.r1~reps*bin*ease, aggregate(mean_v.r1~reps*bin*ease*s, allV1OverTimeM, mean), mean)
# meanV2OverTimeM <- aggregate(mean_v.r2~reps*bin*ease, aggregate(mean_v.r2~reps*bin*ease*s, allV2OverTimeM, mean), mean)
# 
...
# 
# par(mfrow=c(1,1))
# plot(0,0, type='n', xlim=range(meanV1OverTimeM$bin)+c(-.5, .5), ylim=c(0.3, 1), xlab='Bin', ylab='mean EV1')
